...
開いたファイルを履歴に保存する
    　一度開いたVRM、OtherObject、Image、UImageを、アプリ内のストレージに履歴として保存します。

履歴から開いた場合、VRMの確認画面を省略する
    　一度開いたVRMについては、利用条件の確認画面を表示することなく読み込ませるようにします。あとから「詳細情報」ボタンで表示することはできます。

履歴にファイルの参照のみ保存する
    | 　この設定がオフの場合、履歴に保存されるファイルは実体のコピーのため、端末内で移動・削除しても開くことが可能です。ただし端末側で変更がされた場合は最新のファイルは反映されません。
    | 　オンの場合はファイルへの参照のみアプリ内に保存されるためストレージを圧迫しません。そして端末内でファイルが変更されてもアプリで開くと最新のファイルが反映されます。

.. warning::
    | ※この設定はChromium系のブラウザ、デスクトップ版のみ有効です。（Safari、Firefoxではオンにしても使用できません）
    | ※履歴の保存形式が変わるため、設定のオン・オフで今までの履歴は一旦クリアされます。ご注意ください。
