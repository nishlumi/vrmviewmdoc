...
4. 本当に適用してもよい場合は間隔(duration)の入力ボックスで端数を消すなどのキー操作をします。すると変更が確定します。

.. note::
    コピーボタンを押すと間隔の合計値が入力ボックスにセットされますが、その入力をキャンセルしたい場合は入力ボックスでキー操作をせず、タイムラインの選択を切り替えるなどしてください。そうすることで変更がキャンセルされ、別のタイムラインやアバターの編集に移ることができます。