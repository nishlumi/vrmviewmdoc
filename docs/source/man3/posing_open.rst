...
ver 2.18.0より、タイムラインパネルと同じ位置で、メインウィンドウと同じウィンドウ内でポーズやモーションを選んで適用できるようになりました。

パネルを開く
====================================

1. タイムラインパネルのツールバーの左端にある、Pose/Motion ボタンを押します。
...
2. するとポーズ・モーションパネルに切り替わります。

.. image:: ../img/screen_pose4.png
    :align: center

|

元のタイムラインパネルに戻すには、左端の ``閉じるボタン`` を押してください。

.. note::
    パネル表示では、見やすさ重視のためタイトルとサムネイルしか表示していません。それ以外の情報は、ポップアップウィンドウに記載しています。
...
3. 目的のポーズを選び、「適用」をクリックします。またはダブルクリックをします。
...
3. 目的のモーションを選び、「適用」をクリックします。またはダブルクリックをします。
...
パネル表示でダウンロードや削除をするには
===========================================

パネル表示ではモバイル表示時の幅の問題もあり、ツールバーのボタンを減らしています。

以下の2つの機能は、選択したポーズ・モーションの右クリック時のコンテキストメニューから利用できます。
...
Googleドライブ上からの削除について
===========================================

Googleドライブに保存したファイルの削除は、現時点ではGoogleAppsScriptの安全性の考慮のため、アプリ上では実行できないようにしています。

お手数ですが、不要になったポーズやモーションなどは、Googleドライブ上で直接削除をお願いします。
...
* VVM_VVMPOSE_INDEX.csv
* VVM_VVMMOT_INDEX.csv

インデックスファイルの削除後、アプリで再びGoogleドライブにアクセスすると、インデックスファイルが再生成されます。
